...
class Flip_Flop(gym.Env):
    def __init__(self):
        ...
        # self.input_path = "final_project/new_c1.txt"
        ...
        # 初始化 truth table
        self.truth_table = {
            'ff1': {combo: False for combo in self.ff1_actions},
            'ff2': {combo: False for combo in self.ff2_actions}
        }
    ...
    def step(self, action):
        ...
        info = {"episode": {"r": reward, "record": self.record}}

         # 如果環境不使用 truncated，可以將其設置為 False
        truncated = False
    ...
    # Convert chart to arrays
        def convert_chart_to_array(chart, max_length):
            array = np.zeros((max_length, 5), dtype=np.float32)
        ...
        ff1_array = convert_chart_to_array(self.ff1_chart, 76)
        ff2_array = convert_chart_to_array(self.ff2_chart, 22)
